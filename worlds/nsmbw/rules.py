...
def specific_hintmovie_requierments(world: NSMBWworld) -> List:
    ...
    requierments = [
        ...
        [3, rules.Has("Starcoin",5)],  # 03
        [3, rule_completed_everything],  # 04 #find every normal goal in world1-9
        [5, rules.Has("World1_level8_cleared")],  # 05
        [5, rules.Has("Starcoin",10)],  # 06
        [5, rules.Has("Starcoin",30)],  # 07
        [0, rules.Has("Starcoin",15)],  # 08
        [3, rules.Has("Starcoin",1)],  # 09
        [0, rules.Has("Starcoin",95)],  # 10
        [3, rules.Has("Starcoin",150)],  # 11
        [5, rules.Has("Starcoin",1)]  ,# 12
        [5, rule_completed_everything] , # 13 #find every normal adn secret goal in world1-9
        [5, rules.Has("World2_level8_cleared")],  # 14
        [5, rules.Has("Starcoin",215)],  # 15
        [10, rules.Has("Starcoin",25)],  # 16
        [0, rules.Has("Starcoin",65)] , # 17
        [3, rules.Has("Starcoin",35)] , # 18
        [5, rules.Has("Starcoin",165)] , # 19
        [5, rules.Has("Starcoin",190)]  ,# 20
        [0, rules.Has("Starcoin",140)] , # 21
        [3, rules.Has("World3_level8_cleared")],  # 22
        [5, rules.Has("Starcoin",195)] , # 23
        [5, rules.Has("Starcoin",140)],  # 24
        [5, rules.Has("Starcoin",130)] , # 25
        [3, rules.Has("Starcoin",45)]  ,# 26
        [5, rules.Has("Starcoin",175)] , # 27
        [3, rule_completed_everything],  # 28 # everything
        [0, rules.Has("Starcoin",125)],  # 29
        [5, rules.Has("World4_level8_cleared")],  # 30
        [10, rules.Has("Starcoin",70)],  # 31
        [0, rules.Has("Starcoin",50)],  # 32
        [5, rules.Has("Starcoin",69)],  # 33
        [3, rules.Has("Starcoin",145)],  # 34
        [5, rules.Has("Starcoin",105)],  # 35
        [3, rules.Has("Starcoin",55)],  # 36
        [0, rules.Has("Starcoin",75)],  # 37
        [5, rules.Has("World8_level8_cleared")],  # 38
        [3, rules.Has("World5_level8_cleared")],  # 39
        [3, rules.Has("Starcoin",80)],  # 40
        [0, rules.Has("Starcoin",135)],  # 41
        [0, rules.Has("Starcoin",85)] , # 42
        [5, rules.Has("Starcoin",205)],  # 43
        [5, rules.Has("Starcoin",90)] , # 44
        [10, rules.Has("Starcoin",100)] , # 45
        [5, rules.Has("World9_level6_cleared")],  # 46
        [5, rules.Has("World9_level7_cleared")],  # 47
        [0, rules.Has("Starcoin",170)],  # 48
        [0, rules.Has("Starcoin",160)],  # 49
        [3, rules.Has("Starcoin",120)],  # 50
        [3, rules.Has("Starcoin",231)],  # 51
        [0, rules.Has("Starcoin",115)],  # 52
        [3, rules.Has("World8_level8_cleared")],  # 53 #beat world 8 castle
        [5, rules.Has("Starcoin",180)],  # 54
        [0, rules.Has("Starcoin",110)],  # 55
        [5, rules.Has("Starcoin",155)],  # 56
        [5, rule_completed_everything],  # 57 #all secret goals
        [5, rules.Has("Starcoin",225)],  # 58
        [5, rules.Has("Starcoin",220)],  # 59
        [5, rules.Has("Starcoin",185)],  # 60
        [5, rules.Has("Starcoin",210)],  # 61
        [0, rule_completed_everything],  # 62 #all normal goals
        [5, rules.Has("Starcoin",230)],  # 63
        [0, rules.Has("Starcoin",200)],  # 64
...
def specific_level_requierments(world: NSMBWworld) -> List:
    ...
    p_switch = rules.Has(f"movment{p-switch}")
    mushroom = rules.Has(f"movment{Super_Mushroom}")
    requierments = [
        [  # world 1
            [rules.True_(), [propeller, rules.True_(), rules.True_()]],  # -1
            [rules.True_(), [rules.True_(), rules.True_(), rules.Has(f"powerup_state:{"Super_Mushroom"}")]],  # -2
            ...
            [rules.True_(), [rules.True_(), rules.True_(), propeller | rules.Has(f"movment{p-switch}")]],  # -8 1-C
        ],
        [  # world 2
            [rules.True_(), [rules.True_(), rules.True_(), rules.True_()]],  # -1
            [rules.True_(), [rules.True_(), rules.True_(), rules.Has(f"powerup_state:{"Mini_Mushroom"}")]],  # -2
            [rules.True_(), [rules.True_(), rules.True_(), rules.True_()]],  # -3
            [rules.True_(), [rules.True_(), rules.Has(f"powerup_state:{"Propeller_Mushroom"}"), rules.Has(f"powerup_state:{"Propeller_Mushroom"}") | rules.Has(f"powerup_state:{"Mini_Mushroom"}")]],  # -4
            [rules.True_(), [rules.True_(), rules.True_(), rules.True_()]],  # -5
            [rules.True_(), [rules.True_(), rules.True_(), rules.True_()]],  # -6
            [rules.True_(), [rules.True_(), rules.True_(), rules.True_()]],  # -7 2-T
            [ ice_peng | p_switch, [rules.True_(), rules.Has(f"powerup_state:{"Super_Mushroom"}"), propeller | rules.Has(f"movment{p-switch}")]],  # -8 2-C
        ],
        [  # world 3
            [rules.True_(), [rules.True_(), rules.True_(), rules.True_()]],  # -1
            [rules.True_(), [rules.True_(), rules.True_(), rules.True_()]],  # -2
            [rules.True_(), [rules.True_(), rules.True_(), rules.True_()]],  # -3
            [rules.Has(f"movement{"red-block"}"), [rules.True_(), rules.True_(), rules.True_()]],  # -4
            [rules.True_(), [rules.True_(), rules.Has(f"movement{"red-block"}"), rules.Has(f"movement{"red-block"}")]],  # -5
            [rules.True_(), [rules.True_(), rules.True_(), rules.True_()]],  # -6
        ...
        [  # world 4
            [rules.True_(), [rules.True_(), rules.True_(), rules.True_()]],  # -1
            [rules.True_(), [rules.True_(), rules.True_(), rules.True_()]],  # -2
            [rules.True_(), [rules.True_(), rules.True_(), rules.Has(f"powerup_state:{"Mini_Mushroom"}")]],  # -3
        ...
        [  # world 6
            ...
            [rules.True_(), [rules.True_(), rules.True_(), rules.True_()]],  # -8 6-C
...
def set_all_location_rules(world: NSMBWworld) -> None:
    ...
    for world_num in range(1, 9+1):  # worlds
        for level_num in range(1, levels_per_world[world_num - 1]+1):
            ...
            if level_num == 1:
                world.set_rule(flagpole, rules.Has(f"World{world_num}_unlock",1) & level_req[world_num-1][level_num-1][0])
            elif level_num == 4:
                world.set_rule(flagpole, rules.Has(f"World{world_num}_unlock",2) & rules.Has(f"World{world_num}_level{level_num-1}_cleared")& level_req[world_num-1][level_num-1][0])
            else:
            ...
            if world_num == 9:
                world.set_rule(flagpole,rules.Has(f"Starcoin", 24))
    ...
    for hm_num in range(1,HM_COUNT+1):
        ...
        world.set_rule(location, rules.Has(f"Starcoin",total_cost) & hm_req[hm_num-1][1])
