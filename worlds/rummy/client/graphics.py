...
from typing import Literal, NamedTuple, Protocol, cast, Any, Dict
...
## my stuff
IMAGES : Dict[str | RummyCard, RelatedTexture] = {
}
for sym in range(1,MAX_NUMBERS+1):
    IMAGES[RummyCard("BLUE", str(sym))]  = RelatedTexture(r"cards/Cards Pixel Art - Pack (64x96)/black_blue.png",   64*(sym-1), 96*1, 64, 96)
    IMAGES[RummyCard("RED", str(sym))]   = RelatedTexture(r"cards/Cards Pixel Art - Pack (64x96)/black_red.png",    64*(sym-1), 96*2, 64, 96)
    IMAGES[RummyCard("BLACK", str(sym))] = RelatedTexture(r"cards/Cards Pixel Art - Pack (64x96)/black_white.png",  64*(sym-1), 96*3, 64, 96)
    IMAGES[RummyCard("WHITE", str(sym))] = RelatedTexture(r"cards/Cards Pixel Art - Pack (64x96)/white_black.png",  64*(sym-1), 96*4, 64, 96)
    # lowest row (=0) is more fun but weird


def get_rummy_texture(key : Any) -> Texture | None:
    if type(key) == RummyCard:
        return get_texture_by_identifier(IMAGES[key])

    raise ValueError(f"Tried to load unknown key {key}")