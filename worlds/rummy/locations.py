...
LOCATION_NAME_TO_ID.update({ get_merge_name(i): i + 10_000 for i in range(1,MAX_NUMBERS * MAX_COLORS+1)})
LOCATION_NAME_GROUPS.update({"MERGES" : { get_merge_name(i) for i in range(1,MAX_NUMBERS * MAX_COLORS+1)}})
...
def create_regular_locations(world: RummyWorld) -> None:
    world.get_region(RUMMY_REGION).add_locations(get_location_names_with_ids([get_merge_name(i) for i in range(1,MAX_NUMBERS * MAX_COLORS+1)]), RummyLocation)
